...
export default function NotesTab({ currentUser, onRefreshStats }: NotesTabProps) {
  ...
  const sortedNotes = useMemo(() => {
    return [...notes].sort((a, b) => {
      ...
      if (aPinned && bPinned) {
        return getTimeValue(b.pinnedAt || b.updatedAt) - getTimeValue(a.pinnedAt || a.updatedAt);
      }
  ...
  const maxVerseForForm = selectedSurahMeta?.verses || 286;
  ...
  useEffect(() => {
    if (selectedSurahMeta) { 
      if (Number(formVerseNumber) > selectedSurahMeta.verses) {
        setFormVerseNumber(selectedSurahMeta.verses);
      }
    }
  }, [formSurahId]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const maxVerse = SURAHS.find(s => s.id === formSurahId)?.verses || 286;
    const currentVerse = Number(formVerseNumber);
    if (currentVerse < 1 || currentVerse > maxVerse) {
      setErrorMsg(`سورة ${selectedSurahMeta?.name} عدد آياتها ${maxVerse} آية فقط. اختر رقم آية من 1 إلى ${maxVerse}.`);
      return;
    }
  ...
  const handleTogglePin = async (note: QuranNote) => {
    if (!currentUser) return;
    try {
      await updateNote(currentUser.id, note.id, { 
        pinned: !note.pinned,
        pinnedAt: !note.pinned ? serverTimestamp() : null,
      });
      await fetchNotes();
    } catch (err) {
  ...
  const getAyahTextForNote = (note: QuranNote) => {
    const maxVerseForNote = SURAH_VERSE_COUNTS[note.surahId - 1] || 1;
    if (note.verseNumber > maxVerseForNote || note.verseNumber < 1) {
          ...
          {sortedNotes.map((note) => (
            ...
            >
              ...
              <div>
                ...
                {isInvalidVerse && (
                  <div className="p-3 bg-rose-50 dark:bg-rose-950/20 text-rose-700 dark:text-rose-400 rounded-xl text-xs mb-3 flex items-center gap-2">
                    <AlertCircle className="h-4 w-4 flex-shrink-0" />
                    <div>
                      <p className="font-bold">هذه الخاطرة مرتبطة برقم آية غير صحيح.</p>
                      <p>سورة {note.surahName} عدد آياتها {maxVerseForNote} آية فقط.</p>
                      <button onClick={() => handleOpenEditModal(note)} className="mt-2 px-2 py-1 bg-rose-100 dark:bg-rose-900/30 rounded-md text-rose-800 dark:text-rose-300 font-bold hover:bg-rose-200">تصحيح رقم الآية</button>
                    </div>
                  </div>
                )}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <div>
                    <label className="block text-xs text-slate-500 mb-1" >
                      رقم الآية (الحد الأقصى: {maxVerseForForm})
                    </label>
                    <input
                      type="text"
                      inputMode="numeric"
                      ...
                      onBlur={(e) => {
                        const maxVerse = maxVerseForForm;
                      ...
                      id="form-verse-input"
                    />
                    <p className="text-[9px] text-slate-400 mt-1">عدد آيات هذه السورة: {maxVerseForForm} آية</p>
                  </div>
                </div>
