        ...
        <form onSubmit={handleSaveSettings} className="space-y-6">
          ...
          <div className="space-y-4 pt-4 border-t border-slate-100 dark:border-slate-800">
            ...
            <button
              ...
              onClick={async () => {
                try {
                  const { migrateGroupVerseRanges } = await import('../services/firestoreService');
                  alert('جاري التحديث...');
                  const res = await migrateGroupVerseRanges();
                  if (res.success) {
                    alert(`تم التحديث بنجاح! عدد الحلقات التي تم تعديلها: ${res.updatedCount}`);
                  } else {
